...
@st.cache_data
def load_data():
    hourly_data = pd.read_csv("us_market_data_hourly.csv", parse_dates=["observation_date"])
    daily_data = pd.read_csv("us_market_data_daily.csv", parse_dates=["observation_date"])
    return hourly_data, daily_data

hourly_data, daily_data = load_data()

dataset_choice = st.sidebar.selectbox("Select Dataset", ["Hourly Data", "Daily Data"], index=0)
data = hourly_data if dataset_choice == "Hourly Data" else daily_data

st.write(f"### Displaying {dataset_choice}")
st.write(data.head())
...
if st.sidebar.button("Train Model"):
    ...
    ax.legend()
    plt.title(f"{model_choice}: Actual vs Predicted")
    st.pyplot(fig)